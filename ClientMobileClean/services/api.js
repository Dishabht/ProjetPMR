...
export const API_BASE_URL = "https://gzip-notifications-ads-cas.trycloudflare.com";
...
export const registerUser = async (payload) => {
  try {
    const response = await api.post("/users/userAdd", payload);
    return response.data;
  } catch (error) {
    if (error.response) {
      console.log(
...
export const addAccompagnateur = async (payload) => {
  try {
    const response = await api.post("/acc/accAdd", payload);
    return response.data;
  } catch (error) {
    if (error.response) {
      console.log(
...
export const sendConfirmationEmail = async (email, subject, message) => {
  try {
    const response = await api.post("/reservation/sendConfirmationEmail", {
  email,
  subject,
  message,
  } 
